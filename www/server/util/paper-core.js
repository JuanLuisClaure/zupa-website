  ...
  var Base = new function () {
    var hidden = /^(statics|enumerable|beans|preserve)$/,
      ...
      return dst;
        ...
        _each = function _each(obj, iter, bind) {
      if (obj) {
      ...
      for (var i = 0; i < offsets.length && winding.quality < 0.5; i++) {
        ...
        for (var j = 0, l = chain.length; j < l; j++) {
          ...
          if (length <= curveLength) {
            ...
            var wind = !(operator.subtract && path2 && (operand === path1 && path2._getWinding(pt, dir, true).winding || operand === path2 && !path1._getWinding(pt, dir, true).winding)) ? getWinding(pt, curves, dir, true) : {
              winding: 0,
              quality: 1
            };
